...
| ------------------------------------- | ------------------------------------------------------------------------------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `handle_new_user()`                   | —                                                                               | trigger  | **SECURITY DEFINER** (`search_path=public`, EXECUTE revocato a tutti — solo trigger). Inserisce la riga in `public.users` (`visibility_scope = 'restricted'`) quando viene creato un utente in `auth.users`, **e gli concede il grant di tutti i clienti esistenti** (`user_client_access`). |
| `grant_new_client_to_all_users()`     | —                                                                               | trigger  | **SECURITY DEFINER** (`search_path=public`, EXECUTE revocato a tutti — solo trigger). Su INSERT in `clients` (trigger `on_client_created`) concede il nuovo cliente a TUTTI gli utenti esistenti. Evita che un cliente appena aggiunto risulti invisibile. |
